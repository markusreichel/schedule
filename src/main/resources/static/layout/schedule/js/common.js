...
var Message = {
	html : function(type, message){
		return '<div class="alert alert-dismissable alert-' + type + '" role="alert"><span id="alert-message">' + message + '</span><button id="alert-close" type="button" class="close" aria-hidden="true">&times;</button></div>';
	},
	...
	showOrCreate : function(type, message){
		...
		if(alert.length > 0){
			if(alert.filter('div.alert-' + type).length == 0){
				$("div#alert-container div.alert")[0].className='alert alert-' + type;
			}
